...
export function EditGroupModal({ isOpen, onClose, onSubmit, initialName, initialCurrency }: EditGroupModalProps) {
  ...
  const [currency, setCurrency] = useState(getCurrencyCode(initialCurrency));
  const [isLoading, setIsLoading] = useState(false);
  
  // Update state when initial values change (e.g. when opening for a different group)
  useEffect(() => {
    setName(initialName);
    setCurrency(getCurrencyCode(initialCurrency));
    ...
    <ModalPortal>
      ...
      >
        <div className="px-6 py-4 flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800">
          <h2 className="text-xl font-black text-zinc-900 dark:text-white uppercase tracking-tight">Edit Group</h2>
          <button 
            onClick={onClose}
